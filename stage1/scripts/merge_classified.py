"""Merge all 30 classified chunks → unique_dishes_classified.csv (full audit) +
unique_dishes_mains.csv (only keepers, the final dish list)."""
...
CHUNK_DIR = dpath("chunks_classified")
ALL_OUT   = dpath("unique_dishes_classified.csv")
KEEP_OUT  = dpath("unique_dishes_mains.csv")
...
# Write keepers, sorted by count desc
...
print(f"→ wrote {KEEP_OUT} ({len(keep_rows):,} rows, mains only)")
