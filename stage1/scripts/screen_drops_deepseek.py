"""Layer 25 stage 1b — DeepSeek V4 Pro re-screen of Gemini's DROP verdicts only.

Full DeepSeek pass would take 2.6h at observed Pro throughput. Smarter use of
the Pro budget: only re-screen the 30k items Gemini wants to drop. If Pro agrees
they get dropped; if Pro disagrees they're saved as judgment cases.

Inputs:  recipe_screen_gemini.csv (filter to verdict=DROP)
Outputs: recipe_screen_deepseek.csv (resumeable, only the DROP set)
"""
...
JUDGMENTS = dpath("recipe_screen_deepseek.csv")
ENV_FILE  = dpath(".env.openrouter")
ENDPOINT  = "https://openrouter.ai/api/v1/chat/completions"

MODEL       = "deepseek/deepseek-v4-pro"
BATCH_SIZE  = 20
CONCURRENCY = 100        # push higher than Gemini run since each Pro call is slow but parallelizable
RETRIES     = 3
TIMEOUT_S   = 180.0
...
# Load only the Gemini-DROP rows
def load_drop_set():
    rows = []
...
async def judge_batch(client, batch, sem, key, attempt=0):
    async with sem:
        ...
        headers = {
            ...
            "X-Title": "menu-item-impact L25 deepseek-pro drops",
        }
        try:
...
async def main():
    key = load_api_key()
    canonicals = load_drop_set()
    print(f"loaded {len(canonicals):,} Gemini-DROP canonicals to re-screen")
        ...
        async def run_one(batch):
            ...
            async with write_lock:
                ...
                if time.time() - last_print > 10:
                    ...
                    print(f"  {n_done:>6,}/{len(pending):,}  ({n_keep} pro_KEEP-rescued, {n_drop} pro_DROP-confirmed, {n_err} err)  "
    ...
    print(f"  pro_KEEP-rescued (Gemini said DROP, Pro said KEEP): {n_keep:,}")
    print(f"  pro_DROP-confirmed (both said DROP):                {n_drop:,}")
    print(f"  err: {n_err:,}")
